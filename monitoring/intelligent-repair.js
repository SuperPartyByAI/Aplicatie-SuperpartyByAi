...
class IntelligentRepair {
  ...
  async diagnoseFailure(service, healthData) {
    ...
    if (healthData.externalAPIs) {
      const failedAPIs = Object.entries(healthData.externalAPIs)
        .filter(([name, data]) => data.status === 'unhealthy');
  ...
  async selfHeal(service, healthData) {
    ...
    if (healthData.database && healthData.database.latency > 1000) {
      console.log(`⚠️ Slow database (${healthData.database.latency}ms) - optimizing connections...`);
  ...
  getBestFix(failureType) {
    ...
    const bestFix = Object.entries(history.successfulFixes)
      .sort(([, a], [, b]) => b - a)[0];
  ...
  async smartRollback(service) {
    ...
    console.log(`  Rolling back to ${lastGoodVersion.version} (uptime: ${lastGoodVersion.uptime}%)`);
