    ...
    try {
      ...
      if (response.ok) {
        ...
        if (state.responseTimes.length >= 5) {
          ...
          if (avg > 2000) {
            console.warn(`⚠️ ${service.name}: Average response time degrading (${Math.round(avg)}ms)`);
            state.status = 'degraded';
          }
        }
      ...
      console.error(`❌ ${service.name}: ${error.message} (${state.consecutiveFailures}/${this.config.maxConsecutiveFailures})`);
  ...
  printStatus() {
    ...
    this.services.forEach(service => {
      ...
      if (state.repairHistory.length > 0) {
        ...
        console.log(`   Last repair: ${lastRepair.method} (${lastRepair.duration}s) - ${lastRepair.success ? '✅' : '❌'}`);
      }
    });
