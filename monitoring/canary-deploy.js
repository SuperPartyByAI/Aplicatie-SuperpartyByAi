...
class CanaryDeploy {
  ...
  verifyHealth(metrics) {
    if (metrics.errorRate > this.thresholds.errorRate) {
      console.log(`    ❌ Error rate too high: ${metrics.errorRate}% (max: ${this.thresholds.errorRate}%)`);
      return false;
    }
    
    if (metrics.responseTime > this.thresholds.responseTime) {
      console.log(`    ❌ Response time too slow: ${metrics.responseTime}ms (max: ${this.thresholds.responseTime}ms)`);
      return false;
    }
    
    if (metrics.availability < this.thresholds.availability) {
      console.log(`    ❌ Availability too low: ${metrics.availability}% (min: ${this.thresholds.availability}%)`);
      return false;
    }
