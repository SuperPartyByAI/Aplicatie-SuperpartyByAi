  ...
  try {
    ...
    for (const accountDoc of accountsSnapshot.docs) {
      ...
      for (const chatDoc of chatsSnapshot.docs) {
        ...
        const messagesSnapshot = await chatDoc.ref.collection('messages').orderBy('timestamp', 'desc').limit(5).get();
    ...
    sessionsSnapshot.forEach(doc => {
      const data = doc.data();
      console.log(`  ${doc.id}: ${data.metadata?.phone || 'no phone'} (${data.metadata?.status || 'no status'})`);
    });
