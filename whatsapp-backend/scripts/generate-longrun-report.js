...
async function generateReport(days) {
  ...
  // Get probes
  const probesSnapshot = await db.collection('wa_metrics').doc('longrun').collection('probes')
  ...
  const report = {
    ...
    slo: {
      ...
      mttrP95Target: 60,
      verdict: heartbeatCount > 0 && uptimePercent >= 99.0 && probePassRate >= 99.0 ? 'PASS' : (heartbeatCount === 0 ? 'INSUFFICIENT_DATA' : 'FAIL')
    }
  };
