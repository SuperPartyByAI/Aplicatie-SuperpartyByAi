...
async function verifyDoD_WA_1() {
  ...
  try {
    ...
    const status = {
      ...
      lastDisconnectReason: waConnectionDoc.exists ? waConnectionDoc.data().lastDisconnectReason : null,
...
async function verifyDoD_WA_3() {
  ...
  try {
    ...
    const incidentsSnapshot = await db.collection('wa_metrics/longrun/incidents')
...
async function verifyDoD_WA_4() {
  ...
  try {
    const incidentDoc = await db.doc('wa_metrics/longrun/incidents/wa_disconnect_stuck_active').get();
