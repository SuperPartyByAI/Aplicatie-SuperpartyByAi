...
async function verifyOutboundProbe() {
  ...
  const passRate = (outboundProbes.filter(p => p.result === 'PASS').length / outboundProbes.length) * 100;
...
async function verifyInboundProbe() {
  ...
  const passRate = (inboundProbes.filter(p => p.result === 'PASS').length / inboundProbes.length) * 100;
  console.log(`\n✅ Pass rate: ${passRate.toFixed(1)}%`);
