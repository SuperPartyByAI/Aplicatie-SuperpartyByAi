...
function startHeartbeatJob() {
  heartbeatInterval = setInterval(async () => {
    try {
      ...
      await db.collection('wa_metrics').doc('longrun').collection('heartbeats').doc(ts.replace(/[:.]/g, '-')).set({
...
function startRollupJob() {
  ...
  console.log(`📅 Daily rollup scheduled (next in ${Math.floor(msUntilMidnight / 1000 / 60)} minutes)`);
}
...
async function runDailyRollup() {
  try {
    ...
    const heartbeatsSnapshot = await db.collection('wa_metrics').doc('longrun').collection('heartbeats')
    ...
    await db.collection('wa_metrics').doc('longrun').collection('rollups').doc('daily').collection(today.replace(/-/g, '')).doc('summary').set({
      date: today,
      heartbeatCount,
      expectedHeartbeats,
