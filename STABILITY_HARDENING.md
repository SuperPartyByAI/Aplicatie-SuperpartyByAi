...
- **Added:** `FirebaseInitGate` with retry logic (DoD Option 1: 3 total attempts, delays: 10s before attempt 2, 20s before attempt 3)
- **Result:** UI appears immediately; init happens asynchronously with timeout + retry
- **Error UI:** Shows after each failure with attempt counter and Retry button; auto-retry countdown visible
...
- **Verified:** Only `lib/app/app_shell.dart` creates MaterialApp (not in main.dart anymore)
- **Guardrails:** CI checks for both `MaterialApp(` and `MaterialApp.router(` outside approved files
...
- **EvidenceModel:** Returns safe fallback model on invalid data (no app crash)
- **EvidenceService:** Uses `tryFromFirestore` and filters null results
