...
async function requireEmployee(req, res) {
  ...
  if (res.headersSent) return null; // Response already sent
...
exports.getAccounts = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (res.headersSent) return; // Response already sent (401/403)
...
/**
 * POST /whatsappProxy/add-account
 * 
...
exports.addAccount = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Return Railway response unchanged
      return res.status(response.statusCode).json(response.body);
    } catch (error) {
      console.error('[whatsappProxy/addAccount] Error:', error);
      return res.status(500).json({
        success: false,
        error: 'internal_error',
        message: error.message || 'Internal server error',
...
/**
 * POST /whatsappProxy/regenerate-qr/:accountId
 * 
