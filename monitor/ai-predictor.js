...
class AIPrediction {
  ...
  async predictFailures(service, healthData) {
    ...
    if (rtPrediction && rtPrediction.value > this.thresholds.responseTime) {
      const timeToThreshold = this.calculateTimeToThreshold('responseTime', this.thresholds.responseTime);
  ...
  calculateTimeToThreshold(type, threshold) {
    ...
    const currentIndex = data.length - 1;
    const stepsToThreshold = (threshold - model.intercept - model.slope * currentIndex) / model.slope;
