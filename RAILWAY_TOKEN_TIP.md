# ⚠️ Problemă cu legacy hosting Token
...
## Tipuri de Token-uri legacy hosting

### 1. Project Token (Limitare)
- **Creat:** legacy hosting Dashboard → Project → Settings → Tokens
...
- **Creat:** legacy hosting Dashboard → Account Settings → Tokens (sau Team Settings → Tokens)
...
2. **Sau:** legacy hosting Dashboard → Settings → Tokens
...
7. **Rulează script-ul** cu noul token:
   ```bash
   ./setup-legacy hosting-api-direct.sh NEW_PERSONAL_TOKEN
   ```

### Opțiunea 2: Configurează Manual (Cel mai simplu)

Dacă nu vrei să creezi un token nou, configurează manual în legacy hosting Web UI:
