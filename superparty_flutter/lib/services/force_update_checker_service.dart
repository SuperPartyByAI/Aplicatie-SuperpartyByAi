...
class ForceUpdateCheckerService {
  ...
  Future<AppVersionConfig> getVersionConfig() async {
    try {
      ...
      if (data == null || data.isEmpty) {
        debugPrint(
            '[ForceUpdateChecker] ⚠️ Version config data is null or empty');
        debugPrint(
            '[ForceUpdateChecker] ℹ️ Using safe default (force_update=false)');
        return AppVersionConfig.safeDefault();
      }
      ...
      debugPrint(
          '[ForceUpdateChecker]   - min_build_number: ${config.minBuildNumber}');
      debugPrint(
          '[ForceUpdateChecker]   - force_update: ${config.forceUpdate}');
      debugPrint(
          '[ForceUpdateChecker]   - android_download_url: ${config.androidDownloadUrl}');
      ...
      debugPrint(
          '[ForceUpdateChecker] ⚠️ Firebase error: ${e.code} - ${e.message}');
      ...
      debugPrint(
          '[ForceUpdateChecker] ❌ Unexpected error reading version config: $e');
  ...
  Future<bool> needsForceUpdate() async {
    try {
      ...
      debugPrint(
          '[ForceUpdateChecker] Min required build: ${config.minBuildNumber}');
      ...
      if (needsUpdate) {
        ...
        debugPrint(
            '[ForceUpdateChecker] ℹ️ Current: $currentBuildNumber, Required: ${config.minBuildNumber}');
      } else {
      ...
      debugPrint('[ForceUpdateChecker] Stack trace: $stackTrace');
      debugPrint(
          '[ForceUpdateChecker] ℹ️ FAIL-SAFE: App will continue without blocking');
