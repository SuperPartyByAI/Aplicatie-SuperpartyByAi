...
import '../../data/mock_evenimente.dart';
import '../../utils/code_validator.dart';
import 'dovezi_preview_screen.dart';

class EvenimentePreviewScreen extends StatefulWidget {
  const EvenimentePreviewScreen({super.key});

  @override
  State<EvenimentePreviewScreen> createState() => _EvenimentePreviewScreenState();
}

class _EvenimentePreviewScreenState extends State<EvenimentePreviewScreen> {
  ...
  List<EventModel> get _filteredEvents {
    var events = List<EventModel>.from(MockEvenimente.evenimente);
  ...
  Widget _buildExtraFiltersRow() {
    return Row(
      children: [
        Expanded(
          child: TextField(
            enabled: _notedBy == null || _notedBy!.isEmpty,
            onChanged: (value) {
              setState(() {
                _staffCode = value.isEmpty ? null : value;
                if (_staffCode != null) _notedBy = null;
              });
            },
    ...
    return InkWell(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => DoveziPreviewScreen(eventId: event.id),
          ),
        );
      },
      child: Container(
          ...
          children: [
            ...
            if (event.roles.isNotEmpty) ...[
              ...
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: event.roles.map((role) => _buildRoleChip(role)).toList(),
              ),
            ],
  ...
  Widget _buildRoleChip(RoleModel role) {
    Color bgColor;
    Color borderColor;
    switch (role.status) {
