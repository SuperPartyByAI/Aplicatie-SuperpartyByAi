...
class MultiRegion {
  ...
  async monitor() {
    ...
    const betterRegion = this.regions.find(r => 
      r.status === 'healthy' &&
    ...
    if (betterRegion) {
      console.log(`⚡ Found better region: ${betterRegion.name} (${betterRegion.latency}ms vs ${this.activeRegion.latency}ms)`);
  ...
  getStats() {
    ...
    const avgFailoverTime = totalFailovers > 0 ?
      this.failoverHistory.reduce((sum, f) => sum + f.duration, 0) / totalFailovers : 0;
